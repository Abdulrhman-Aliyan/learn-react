...
export function ChallengePanel({ challenge, results, hasRun, passed }) {
  ...
  return html`
    <div className="space-y-3">
      <div>
        <span className="font-mono text-[10px] uppercase tracking-[0.15em] text-slate-500">challenge</span>
        <h4 className="mt-1 text-[15px] font-semibold text-slate-900">${challenge.title}</h4>
      </div>

      <ul className="space-y-1.5 text-[13.5px] leading-relaxed text-slate-700">
      ...
      ${passed ? html`
        <div className="rounded-lg border border-green-300 bg-green-50 p-3">
          <p className="text-[13.5px] font-medium text-green-900">Solved. Every check passes.</p>
        </div>
      ` : null}

      ${hasRun && failing.length ? html`
        <div className="space-y-2">
          <div className="font-mono text-[10px] uppercase tracking-[0.15em] text-slate-500">checks</div>
          ${results.map(function (c, i) {
            return html`
              <div key=${i} className="flex gap-2">
                <span className=${'mt-0.5 shrink-0 text-[12px] ' + (c.passed ? 'text-green-600' : 'text-slate-300')}>
                  ${c.passed ? '✓' : '○'}
                </span>
                <div className="min-w-0 flex-1">
                  <div className=${'text-[13px] ' + (c.passed ? 'text-slate-400 line-through decoration-slate-300' : 'text-slate-800')}>
                    ${c.label}
                  </div>
                  ${!c.passed ? html`
      ...
      ${hints.length ? html`
        <div className="rounded-lg border border-slate-200 bg-white p-3">
          ...
          ${shown === 0 ? html`
            <p className="mt-2 text-[12.5px] text-slate-400">Try it cold first. The hints are here when you want them.</p>
          ` : null}

          ${hints.slice(0, shown).map(function (h, i) {
            return html`
              <div key=${i} className="mt-2.5 border-l-2 border-slate-200 pl-2.5">
                <div className="font-mono text-[10px] uppercase tracking-[0.15em] text-slate-400">${RUNG_LABEL[i] || ('hint ' + (i + 1))}</div>
                <div className="mt-1 text-[13px] leading-relaxed text-slate-700"><${Rich}>${h}<//></div>
              </div>`;
          })}
        </div>
      ` : null}
