...
export default {
  ...
  guided: {
    ...
    starter: [
      ...
      "//",
      "// Run this first. Nothing on the screen responds: the queue is a plain",
      "// variable, and React never hears about a plain variable changing.",
      "//",
      "// Each step below replaces exactly one marked line.",
      "",
      "const SEED = [",
      ...
      "",
      "function App() {",
      "  let jobs = SEED;                                        // STEP 1",
      "",
      "  let filter = '';                                        // STEP 2",
      "",
      "  const visible = jobs;                                   // STEP 4",
      "",
      "  function markDone(id) {",
      "    jobs.find(j => j.id === id).status = 'done';          // STEP 5",
      "  }",
      "",
      "  return (",
      "    <div className=\"space-y-3\">",
      "      <input value=\"\" placeholder=\"Filter by filename...\" className=\"w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900\" />{/* STEP 3 */}",
      "",
